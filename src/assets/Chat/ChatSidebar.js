const ChatSidebar = ({ conversations, setActiveConversation, activeConversation }) => {
    return (
        <div style={{ width: 250, borderRight: "1px solid #ccc", overflowY: "auto" }}>
            <h4 style={{ padding: 10 }}>Rozmowy</h4>
            {conversations.map((conv, index) => {
                const isActive = activeConversation?.id_pet === conv.id_pet &&
                ...
                return (
                    ...
                    >
                        ...
                        <strong>Rozmowa z:</strong> {conv.conversation_with_name || `Użytkownik ${conv.conversation_with}`}
                    </div>
                );
            })}
        </div>
